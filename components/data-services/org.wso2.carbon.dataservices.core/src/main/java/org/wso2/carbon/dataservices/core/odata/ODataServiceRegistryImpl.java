 ...
 */
public class ODataServiceRegistryImpl implements ODataServiceRegistry {
	private static Log log = LogFactory.getLog(ODataServiceRegistryImpl.class);

	private static ODataServiceRegistryImpl serviceRegistry;

	private Map<String, ConcurrentHashMap<String, ODataServiceHandler>> registry = new ConcurrentHashMap<>();

	public ODataServiceRegistryImpl() {
...
/*
	public static void startFlow(HttpRequest request) {
		ODataServiceRegistryImpl registry = getInstance();
		registry.getServiceHandler().process(registry);
	}
*/

	public static synchronized ODataServiceRegistryImpl getInstance() {
		if (serviceRegistry == null) {
			serviceRegistry = new ODataServiceRegistryImpl();
		}
